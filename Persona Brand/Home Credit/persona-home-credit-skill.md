# Home Credit Persona: Nguyên tắc viết bài Blog HCVN

## 1. Từ khóa (Keywords)

### Cấu trúc từ khóa
- Mỗi bài có **1 từ khóa chính** + **4–5 từ khóa phụ**.
- Từ khóa chính xuất hiện **xuyên suốt bài**.
- Từ khóa phụ xuất hiện **ít nhất 1 lần** trong bài.

### Mật độ từ khóa
- **Mật độ từ khóa**: 1% – 1.5% (tổng số từ khóa / tổng số từ).

---

## 2. Title (Thẻ tiêu đề)
- **Bắt buộc** chứa từ khóa chính.
- Tối đa **55 – 60 ký tự** (tính cả space).
- Check độ trùng lặp title bằng cú pháp: `allintitle:title` (search trên Google).

---

## 3. Meta Description
- Nằm trong khoảng **130 – 160 ký tự** (tính cả space).
- **Bắt buộc** chứa từ khóa chính.
- Ngắn gọn, tóm tắt ý chính nội dung bài (có thể là tóm tắt hoặc dẫn dắt).

---

## 4. Intro (Mở bài)
- Nằm trong khoảng **70 – 150 chữ**.
- **Bắt buộc** chứa từ khóa chính (tốt nhất nên nằm trong **100 từ đầu tiên**).
- Giới thiệu ngắn gọn về topic của bài.

---

## 5. Headings (Tiêu đề phụ)
- Tối thiểu **2–3 H2** (nên có H3).
- **Bắt buộc 1–2 headings** chứa từ khóa chính.
- **Hạn chế** propose Heading dưới dạng câu hỏi.

---

## 6. Nội dung bài viết

### Chất lượng
- **Không dùng spinning tool**, độ unique > 90%.
- Ngắn gọn, dễ hiểu, đúng sự thật (research kỹ trước khi viết).

### Định dạng đoạn văn
- Mỗi đoạn văn **không quá 4 dòng**.
- Giới hạn mỗi câu văn: **tối đa 25 chữ**.

### FAQs (Câu hỏi thường gặp)
- **Bắt buộc** có phần FAQs.
- Các câu hỏi phải dựa trên từ khóa chính hoặc phụ.
- User phải có nhu cầu giải đáp → không propose câu không liên quan.

### Disclaimer (Bắt buộc)
Chèn **ở cuối bài, trước phần CHỮ KÝ**:
> *Lưu ý: Thông tin trong bài mang tính chất tham khảo, được tổng hợp từ thị trường và không đại diện cho toàn bộ sản phẩm, dịch vụ của Home Credit.*

### Kết bài
- **Bắt buộc** chứa từ khóa chính.
- Có **CTA** phù hợp.
- Có **chữ ký cuối bài** (HCVN_BLOG SIGNATURE, format in nghiêng tất cả).

### Độ dài bài viết
- Cột mốc bài chuẩn: **Tối thiểu 2500 từ** (áp dụng tiêu chuẩn nội dung chuyên sâu).

---

## 7. Hình ảnh

### Thông số kỹ thuật
| Loại | Kích thước | Ghi chú |
|---|---|---|
| Ảnh trong bài | 1920 × 1080 | Dung lượng tối đa 150 KB/ảnh |
| Thumbnail | 840 × 480 | Bắt buộc có |
| Banner | 1440 × 480 | Bắt buộc có |

- **Định dạng**: WebP.
- Bài 2500+ từ → tối thiểu **8–10 hình ảnh**.

### Quy tắc chọn ảnh
- **KHÔNG tự động chèn/add hình ảnh vào nội dung bài viết.** (Team thiết kế sẽ đảm nhiệm riêng khâu này)
- Vẫn có thể đề xuất các vị trí đặt ảnh vào file notes nếu cần, nhưng không trực tiếp nhúng file ảnh/URL ảnh vào file content.

### Tool resize/compress ảnh
- [Squoosh](https://squoosh.app)

---

## 8. Internal Link (Liên kết nội bộ)

### Nguyên tắc bắt buộc
- **Bắt buộc** chèn link nội bộ (domain `www.homecredit.vn`).
- **Không** chèn link external (link ra ngoài website khác).
- Mỗi link chỉ chèn **1 lần** trong 1 bài.

### Mật độ & phân bổ
- Mật độ internal link: **0.8% – 1.2%**.
- **Không** đặt mật độ link dày đặc 1 chỗ (không để 1–2–3–4–5 link đầu bài, giữa bài hay cuối bài).
- Bài viết liên quan đặt ở **thân bài**, trong context thật sự liên quan.

### Cách đi link
- Đi link theo đúng **intent người dùng**: khi user lướt đến đoạn thông tin cần biết thêm → dẫn link về bài liên quan (đúng context).
- Đi link theo dạng **anchor text** trong một ngữ cảnh liên quan, KHÔNG đi theo cụm 1–2–3–4 link.
- **Xử lý Anchor Text tự nhiên**: Bắt buộc phải lược bỏ các cụm từ hỏi như "là gì", "như thế nào" khi chèn link để câu văn trong bài luôn mượt mà. (Ví dụ: thay vì viết bắt buộc *hiểu rõ [phí thường niên là gì](link)*, hãy viết tự nhiên thành *gánh nặng [phí thường niên](link)*).
- **Ví dụ**: Bài viết đang ở đoạn "Điều kiện mở thẻ tín dụng online" → có yêu cầu KH cần có điểm tín dụng tốt → hyperlink về bài "điểm tín dụng".

---

## 9. Tool kiểm tra chất lượng

| Mục đích | Tool |
|---|---|
| Đếm ký tự | Google Translate, Word Counter, Google Docs (`Ctrl+A` → `Ctrl+Shift+C`) |
| Check unique | SmallSEOTools, Plagiarism Checker, Search Engine Reports |
| Resize/compress ảnh | [Squoosh](https://squoosh.app) |
| Check trùng title | `allintitle:title` trên Google |

---

## 10. Hướng dẫn cho Assistant

Khi viết bài cho Home Credit Blog:

1. **Trước khi viết**: Xác nhận 1 từ khóa chính + 4–5 từ khóa phụ.
2. **Title**: Chứa từ khóa chính, đếm ≤ 60 ký tự.
3. **Meta Description**: Chứa từ khóa chính, 130–160 ký tự.
4. **Intro**: 70–150 chữ, từ khóa chính trong 100 từ đầu.
5. **Headings**: Tối thiểu 2–3 H2, 1–2 heading chứa từ khóa chính, hạn chế dạng câu hỏi.
6. **Nội dung**: Đoạn ≤ 4 dòng, câu ≤ 25 chữ, unique > 90%.
7. **FAQs**: Bắt buộc, dựa trên từ khóa chính/phụ.
8. **Disclaimer**: Chèn trước chữ ký cuối bài.
9. **Internal link**: Chỉ domain `homecredit.vn`, mật độ 0.8%–1.2%, phân bổ tự nhiên theo context.
10. **Hình ảnh**: KHÔNG tự động chèn hình ảnh vào nội dung bài viết. Đoạn nào cần, chỉ cần để placeholder (VD: `[Chèn ảnh...]`).
11. **Mật độ từ khóa**: Kiểm tra đạt 1%–1.5%.
12. **Chữ ký cuối bài**: Chọn đúng mẫu signature theo chủ đề sản phẩm (xem Mục 11).

---

## 11. Chữ ký cuối bài (HCVN_BLOG SIGNATURE)

> **Quy tắc chung**: Toàn bộ chữ ký phải được **in nghiêng**. Chọn đúng mẫu signature phù hợp với chủ đề bài viết.

### Signature 1: Cash Loan – Vay tiền mặt

> *Với lãi suất cạnh tranh và minh bạch, vay tiền mặt từ Home Credit giúp khách hàng giải quyết các nhu cầu tài chính tức thời cùng những ưu điểm sau:*
>
> - *Hạn mức vay linh hoạt: Từ 5 đến 250 triệu đồng, đáp ứng đa dạng nhu cầu khách hàng*
> - *Thủ tục đơn giản: Chỉ cần CCCD/Thẻ Căn cước còn hiệu lực là khách hàng đã có thể đăng ký vay trực tuyến ở bất kỳ đâu*
> - *Duyệt vay siêu tốc: Duyệt nhanh 3 phút, hoàn toàn online*
> - *Giải ngân nhanh: Nhận tiền trong 10 phút ngay sau khi hoàn tất ký hợp đồng*
>
> *[Đăng ký vay ngay!](https://www.homecredit.vn/vay-tien-online-nhanh/dang-ky)*

### Signature 2: Two Wheel – Vay trả góp xe máy

> *Home Credit mang đến giải pháp vay trả góp xe máy với quy trình vay tiện lợi, hỗ trợ khách hàng sở hữu chiếc xe máy yêu thích dễ dàng hơn bao giờ hết. Theo đó, khách hàng có thể đăng ký vay và hưởng các tiện ích như:*
>
> - *Khoản vay linh hoạt từ 5 – 100 triệu đồng*
> - *Thời gian trả góp từ 6 - 36 tháng*
> - *Lãi suất ưu đãi, chỉ từ 0%*
> - *Nhận cà vẹt gốc nếu có điểm tín dụng tốt*
> - *Thủ tục đơn giản, chỉ cần có CCCD/Thẻ Căn cước chính chủ còn hiệu lực, không chứng minh thu nhập, không thẩm định người thân*
>
> *Khám phá ngay tại: [Mua trả góp xe máy qua Home Credit - Trả trước 0Đ](https://www.homecredit.vn/mua-xe-may-tra-gop)*
>
> *Hotline tư vấn sản phẩm: 1900 633 633*

### Signature 3: Consumer Durables – Vay trả góp điện tử, điện máy

> *[Vay trả góp điện tử, điện máy](https://www.homecredit.vn/vay-tra-gop-thiet-bi-dien-tu) từ Home Credit*
>
> *Gói vay trả góp điện tử, điện máy của Home Credit hỗ trợ khách hàng mua trả góp các sản phẩm: điện thoại, laptop, máy tính bảng, tivi, tủ lạnh, máy lọc nước và phụ kiện điện tử tại các cửa hàng liên kết với nhiều ưu đãi hấp dẫn:*
>
> - *Khoản trả góp từ 1 - 40 triệu đồng*
> - *Kỳ hạn thanh toán từ 4 – 24 tháng*
> - *Lãi suất chỉ từ 0%*
> - *Chỉ cần CCCD/Thẻ Căn cước, không chứng minh thu nhập, không thẩm định người thân*
>
> *Khám phá ngay tại: [Vay trả góp điện thoại, điện máy Home Credit lãi suất từ 0%](https://www.homecredit.vn/vay-tra-gop-thiet-bi-dien-tu)*
>
> *Hotline tư vấn sản phẩm: 1900 633 633*

### Signature 4: Credit Card – Thẻ tín dụng

> *Thẻ tín dụng Home Credit là sản phẩm tài chính được công ty tài chính Home Credit và công ty công nghệ thanh toán VISA kết hợp phát hành. Đây là dòng thẻ cho phép khách hàng thực hiện giao dịch mua hàng tại điểm chấp nhận thẻ, mua sắm trực tuyến và trong hạn mức tín dụng được cấp bởi Home Credit tại máy ATM có logo Visa. Ngoài ra, khách hàng được đăng ký trả góp với lãi suất 0% ngay trên ứng dụng Home Credit.*
>
> *Tìm hiểu thêm thông tin về sản phẩm và biểu phí thẻ: [Mở thẻ tín dụng Home Credit online không chứng minh tài chính](https://www.homecredit.vn/the-tin-dung-online)*
>
> *Hotline tư vấn sản phẩm thẻ: 1900 633 999*

### Signature 5: VAS – Bảo hiểm

> *Giải pháp bảo hiểm toàn diện từ Home Credit giúp khách hàng an tâm trước mọi rủi ro về sức khỏe, tài chính và tài sản trong cuộc sống. Với đa dạng lựa chọn về quyền lợi và mức phí, Home Credit mang đến các gói bảo hiểm bao gồm:*
>
> - *[Gói An tâm tài chính](https://www.homecredit.vn/bao-hiem/goi-an-tam-tai-chinh)*
> - *[Bảo hiểm Bảo vệ Xe máy Toàn diện](https://www.homecredit.vn/bao-hiem/bao-hiem-bao-ve-xe-may-toan-dien)*
> - *[Bảo hiểm Trang thiết bị Nội thất](https://www.homecredit.vn/bao-hiem/bao-hiem-trang-thiet-bi-noi-that)*
> - *[Bảo hiểm Bảo vệ Toàn diện Thiết bị di động](https://www.homecredit.vn/bao-hiem/bao-hiem-bao-ve-toan-dien-thiet-bi-di-dong)*
> - *[Bảo hiểm Tai nạn Cá nhân](https://www.homecredit.vn/bao-hiem/bao-hiem-tai-nan)*
> - *[Bảo hiểm Sức khỏe Nội trú](https://www.homecredit.vn/bao-hiem/bao-hiem-suc-khoe-noi-tru)*
> - *[Bảo hiểm Sức khỏe Toàn diện](https://www.homecredit.vn/bao-hiem/bao-hiem-suc-khoe-toan-dien)*
> - *[Bảo hiểm Bệnh hiểm nghèo](https://www.homecredit.vn/bao-hiem/bao-hiem-benh-hiem-ngheo)*
> - *[Bảo hiểm Rơi vỡ Màn hình](https://www.homecredit.vn/bao-hiem/bao-hiem-roi-vo-man-hinh)*
>
> *Mỗi gói bảo hiểm từ Home Credit mang đến những quyền lợi ưu việt, giúp bạn yên tâm tận hưởng cuộc sống mà không còn lo lắng bởi gánh nặng tài chính. Gọi ngay 1900 633 633 để được tư vấn và giải đáp mọi thắc mắc chi tiết!*

### Signature 6: Home PayLater – Mua trước trả sau

> *[Tài khoản trả sau Home PayLater](https://www.homepaylater.vn/)*
>
> *Tài khoản trả sau Home PayLater là một phương thức thanh toán thông minh, được phát triển bởi Home Credit, hỗ trợ người dùng mua trước trả sau. Khách hàng có thể đăng ký tài khoản Home PayLater 100% online với hạn mức tối đa 25 triệu để mua trước trả sau tại hơn 1.000 đối tác trên khắp cả nước, chia nhỏ thanh toán, trả dần trong 1, 3, 6, 12 tháng với lãi suất chỉ từ 0%.*
>
> *Chỉ với 60 giây, không cần chứng minh thu nhập, [ĐĂNG KÝ](https://homepaylater.vn/mp/dang-ky?utm_source=homepaylater&utm_medium=blog) Tài khoản trả sau Home PayLater ngay để nhận ưu đãi mỗi ngày, thỏa thích mua sắm tại hơn +700.000 điểm chấp nhận thanh toán trên toàn quốc và tại các đối tác uy tín như: Thế Giới Di Động, Điện Máy XANH, AVAKids, Traveloka, Tiki...*
>
> *Mọi thắc mắc và thông tin chi tiết vui lòng liên hệ:*
> - *Hotline: 1900 633 999*
> - *[Tải ứng dụng](https://www.homecredit.vn/ung-dung-home-credit)*

### Signature 7: Cẩm nang Tài chính

> *Blog Home Credit - Cẩm nang Tài chính số Toàn diện*
>
> *Tài chính không chỉ là những con số mà còn là chìa khóa mở ra sự tự chủ và ổn định trong cuộc sống. Với Blog Cẩm nang Tài chính số Toàn diện, Home Credit sẽ giúp bạn:*
>
> - *Nắm vững kiến thức về tín dụng và nợ để tránh các rủi ro tài chính*
> - *Hiểu rõ kiến thức tài chính tổng quan và cách quản lý tài chính cá nhân hiệu quả*
> - *Học cách quản lý chi tiêu và kiểm soát dòng tiền, từ kiếm tiền, tiết kiệm cho đến đầu tư theo năng lực, hướng đến độc lập và tự do tài chính*
> - *Thực hiện thanh toán dễ dàng, an toàn, từ mua sắm đến các hóa đơn thiết yếu*
>
> *Chủ động hiểu về tài chính cũng chính là cách để bạn vững tâm trước mọi quyết định lớn nhỏ. Truy cập trang [Cẩm nang Tài chính số Toàn diện](https://www.homecredit.vn/blog) của Home Credit ngay hôm nay!*

### Signature bổ sung: Ứng dụng Home Credit

> *[Ứng dụng Home Credit](https://www.homecredit.vn/ung-dung-home-credit)*
>
> *Ứng dụng Home Credit do công ty tài chính Home Credit phát triển. Được giám sát và quản lý chặt chẽ bởi Ngân hàng Nhà nước Việt Nam, ứng dụng Home Credit sẽ là người bạn đáng tin cậy giúp khách hàng đăng ký và quản lý khoản vay hiệu quả.*
>
> *Với ứng dụng Home Credit, khách hàng có thể đăng ký vay tiền mặt, thanh toán khoản vay, thanh toán hóa đơn điện, nước và nạp tiền điện thoại, quản lý khoản vay, cập nhật các chương trình khuyến mãi,... một cách dễ dàng và nhanh chóng. Ngoài ra, khách hàng có thể sử dụng ứng dụng để quét mã QR với Tài khoản trả sau Home PayLater thanh toán tại hơn 700.000 điểm chấp nhận thanh toán toàn quốc cực kỳ tiện lợi.*
>
> *Mọi thắc mắc và thông tin chi tiết vui lòng liên hệ: 1900 633 999*

---

## 12. Bảng tra cứu nhanh — Chọn Signature theo chủ đề

| Chủ đề bài viết | Signature sử dụng |
|---|---|
| Vay tiền mặt, vay online, vay nhanh | **Signature 1** – Cash Loan |
| Vay trả góp xe máy, mua xe máy | **Signature 2** – Two Wheel |
| Mua trả góp điện thoại, laptop, điện máy | **Signature 3** – Consumer Durables |
| Thẻ tín dụng, mở thẻ, biểu phí thẻ | **Signature 4** – Credit Card |
| Bảo hiểm sức khỏe, tai nạn, tài chính | **Signature 5** – VAS Bảo hiểm |
| Mua trước trả sau, Home PayLater, BNPL | **Signature 6** – Home PayLater |
| Kiến thức tài chính, quản lý chi tiêu, tín dụng | **Signature 7** – Cẩm nang Tài chính |
| Ứng dụng Home Credit, quản lý khoản vay | **Signature bổ sung** – Ứng dụng HC |
